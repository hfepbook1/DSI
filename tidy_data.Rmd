...
```{r, error = TRUE}
pulse_data = read_sas("~/Desktop/public_pulse_data.sas7bdat") %>%
  clean_names() %>%
...
litters_data = 
  ...
  select(litter_number, dose, day_of_tx, sex = sex_ch, gd_of_dob, pd_walk) %>%
...
```{r}
fellowship = read_excel("./data/LotR_Words.xlsx", range = "B3:D6") %>%
  clean_names() %>%
...
return = read_excel("./data/LotR_Words.xlsx", range = "J3:L6") %>%
  clean_names() %>%
  gather(key = sex, value = words, female:male) %>%
  mutate(race = tolower(race),
         movie = "Return")
...
```{r}
lotr_tidy = bind_rows(fellowship, two_towers, return) %>%
...
As an example, consider the data tables in `FAS_pups.csv` and `FAS_litters.csv`, which are related through the `litter_id`. The former contains data unique to each pup, and the latter contains data unique to each litter. We can combine these using a left join of litter data into pup data; doing so retains data on each pup and adds data in new columns.

```{r, error = TRUE}
pup_data = 
litter_data = 

FAS_data = left_join(pup_data, litter_data, by = "litter_id")
FAS_data
```

Note that we made the key explicit in the join. By default, the `*_join` functions in `dplyr` will try to determine the key(s) based on variable names in the datasets you want to join. This is often but not always sufficient, and an extra step to make the key clear will help you and others reading your code.
