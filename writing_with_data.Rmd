...
**_Learning assessment:_** Write a named code chunk that creates a variable containing a sample of size 500 from a random normal variable, takes the absolute value, and produces a histogram of the resulting variable. Add an inline summary giving the median value. What happens if you set `eval = FALSE` to the code chunk? What about `echo = FALSE`? 
...
**_Learning assessment:_** After the preceding code chunk, write a bullet list given the mean, median, and standard deviation of your random sample. 
...
**_Learning assessment:_** Convert the scripts for exploring vector classes and producing basic plots (from [best practices](best_practices.html)) to self-contained R Markdown files. 
