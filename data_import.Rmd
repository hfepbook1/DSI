...
We're going to figure out how to import the three datasets in this zip file. First, create a directory for today's work, add an R Project, move the data to your directory, and start an R Markdown file. We're also going to be making extensive use of the `tidyverse` package, so we can go ahead and load that.
...
Another tool that I use often is ``skimr::skim`. The output is too wide to render well here, but is pretty slick!
```{r}
...
```{r, error = TRUE}
library(haven)
pulse_data = read_sas("data/pulse.sas7bdat")
head(pulse_data, 5)
```
