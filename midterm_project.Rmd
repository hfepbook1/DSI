...
Study participants were recruited from 50 Head Start centers in northern Manhattan, the Bronx, and Brooklyn, in neighborhoods with high rates of pediatric asthma. Researchers used a survey instrument to collect data on the child’s age, race, sex, asthma symptoms and other medical conditions, birth order and family-related factors, and features of the home environment.  Field staff measured the child’s height, weight, and skin-fold thicknesses The staff then attached the accelerometer to the child’s non-dominant wrist with a hospital band. We obtain minute-by-minute activity data by averaging, for each minute separately, across the six days of observation for each child. Data are additionally aggregated into 10-minute epochs by average across minutes in 10-minute intervals.
...
You're asked to submit a zipped R Project that contains a reproducible report covering the topics below. Do not include raw data files in your directory. Instead, create a separate directory called `data` and use relative paths starting with `../data/` to load data. 
