...
Once again we're going to be using the `tidyverse`, so we'll load that at the outset. We're going to be looking at a lot of output, so we'll print only three lines of each tibble by default.  Lastly, we'll focus on the data in FAS_litters.csv dataset, so we'll load those data and clean up the column names using what we learned in [data import](data_import.html).
